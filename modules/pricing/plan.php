...
if(isset($_POST['submit']) && $_POST['submit']==1)
{
	...
	if($storename=="")
		...
		$error="Store name must be entered...";

	else if(unique("member","store_name",$storename)==false)

		$error="Store name already present...";		

	else if($username=="")

		$error="User name must be entered...";	
		...
		$error="Address one must be entered...";

	else if($address2=="")

		$error="Address two must be entered...";
		...
		$error="Zip code must be entered...";
		...
		$error="Phone number must be in digit...";
		...
		$error="Password does not matched, Enter correct password as above...";
...

	{
		...
		$insert=array("user_name"=>$username,"full_name"=>$fullname,"password"=>$password,"store_name"=>$storename,"address"=>$address1,"location"=>$location,"zipcode"=>$zipcode,"email"=>$email,"phone"=>$phoneno,"new_user_id"=>$new_user_id);
