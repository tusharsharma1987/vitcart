...
<?php
	if(isset($_GET['id']) && $_GET['id']!="")
	{
		...
		$check=mysql_query("select * from static_pages where user_id='0' and page_name='".$id."'") or die(mysql_error());
	...
	{	
		...
		{
			$row=mysql_fetch_array($check);
			$update_poem=mysql_query("update static_pages set content='".$content."' where id = '".$row['id']."'") or die(mysql_error());		
			?>
		...
		{
			$insert_cms=mysql_query("insert into static_pages (page_name,content,user_id) values ('".$id."','".$content."','0')") or die(mysql_error());		
			?>
			<script language="javascript">
				window.location.href="message.php?msg=11";
			</script>
...
<script type="text/javascript" src="ckeditor/ckeditor.js"></script>
	<div align="center" style="min-height:500px; text-align:center">
		...
		<div style="float:left;  padding-left:10px; "><textarea  name="poem" rows="50" id="poem_text" style="width:500px; height:500px"><?php if(isset($_GET['id'])){echo $row['content']; } ?></textarea></div>
		<div style="clear:both">&nbsp;</div>
