...
if(isset($_POST['submit']) && $_POST['submit'] == "submit")
	...
	elseif($npwd==$opwd)
		...
		{
		$sql_check=mysql_query("select * from admin_user where id =1") or die(mysql_error());
		$data_admin=mysql_fetch_array($sql_check);
		$password=$data_admin['password'];
		
		if($password != md5($opwd))
			{
				$error = "Old Password is not Correct";
			}
		else
			{
			$sql_chamge=mysql_query("update admin_user set password = '".md5($npwd)."' where id=1") or die(mysql_error());
			?>
	...
	<div style="width:600px; padding-left:400px; ">
     <form action="" method="post" enctype="multipart/form-data" name="form2" id="form2" onSubmit="return validate();">
			  <table width="80%" height="142" border="0" align="center" style="padding-top:20px">
                <tr>
                  <td width="30%" height="34" ><span style="font-size:12px">ENTER OLD PASSWORD: </span></td>
                ...
                <tr>
                  <td height="29"  ><span style="font-size:12px">ENTER NEW PASSWORD: </span></td>
                ...
                <tr>
                  <td height="27" ><span style="font-size:12px">CONFIRM NEW PASSWORD: </span></td>
